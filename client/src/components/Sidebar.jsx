...
const settingItems = [
  ...
  { label: 'General', to: '/settings/general', icon: (
...
const adminItem = { label: 'Users', to: '/users', icon: (
...
export default function Sidebar({ open, onClose }) {
  ...
  return (
    <>
      ...
      {/* Sidebar */}
      <aside
        className={`fixed inset-y-0 left-0 z-30 w-72 bg-slate-900 flex flex-col transform transition-transform duration-300 ease-in-out
          lg:relative lg:w-64 lg:translate-x-0 lg:flex-shrink-0
          ${open ? 'translate-x-0 shadow-2xl' : '-translate-x-full'}`}
      >
        {/* Logo */}
        <div className="flex items-center justify-between h-16 px-4 border-b border-slate-800 flex-shrink-0">
          <div className="flex items-center gap-2.5">
            <div className="w-8 h-8 rounded-lg bg-indigo-500 flex items-center justify-center font-bold text-white text-sm shadow-sm">LF</div>
        ...
        <nav className="flex-1 px-3 py-4 space-y-0.5 overflow-y-auto overscroll-contain">
          {navItems.map((item) => <NavItem key={item.to} item={item} onClose={onClose} />)}
          ...
          {user?.role === 'admin' && (
            <>
              ...
              <NavItem item={adminItem} onClose={onClose} />
            </>
          )}
          ...
          {settingItems.map((item) => <NavItem key={item.to} item={item} onClose={onClose} />)}
        </nav>
        ...
        <div className="p-3 border-t border-slate-800 flex-shrink-0">
          ...
          <button
            ...
            className="w-full flex items-center gap-2.5 px-3 py-2.5 text-sm text-slate-400 hover:text-red-400 hover:bg-slate-800 rounded-xl transition-colors"
          >
